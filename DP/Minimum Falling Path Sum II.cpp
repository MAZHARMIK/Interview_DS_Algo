...
public:
    ...
    int solve(int row, int col, vector<vector<int>>& grid, map<pair<int, int>, int>& mp) {
        if (row == n - 1) {
            return grid[row][col];
        }
        ...
        for (int nextCol = 0; nextCol < n; nextCol++) {
            if (nextCol != col) {
                ans = min(ans, solve(row + 1, nextCol, grid, mp));
            }
        }

        mp[{row, col}] = grid[row][col] + ans;
        return mp[{row, col}];
    }
    ...
    int minFallingPathSum(vector<vector<int>>& grid) {
        ...
        for (int col = 0; col < n; col++) {
            result = min(result, solve(0, col, grid, mp));
        }
